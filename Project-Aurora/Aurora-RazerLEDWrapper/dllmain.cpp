...
Logitech_keyboardBitmapKeys ToLogitechBitmap(int rzrow, int rzcolumn)
{
	...
	if (rzrow == 0 && rzcolumn == 1)
		...
		return Logitech_keyboardBitmapKeys::BITLOC_PAUSE_BREAK; //Not implemented on Logitech
	else if (rzrow == 0 && rzcolumn == 20)
		return Logitech_keyboardBitmapKeys::LOGO; //Row 2
		...
		return Logitech_keyboardBitmapKeys::BITLOC_NUM_MINUS; //Row 3
		...
		return Logitech_keyboardBitmapKeys::BITLOC_NUM_PLUS; //Row 4
		...
		return Logitech_keyboardBitmapKeys::BITLOC_NUM_SIX; //Row 5
		...
		return Logitech_keyboardBitmapKeys::BITLOC_NUM_ENTER; //Row 6
...
bool __fastcall WriteToPipe(WRAPPER_EFFECT effect)
{
	...
	if (!requiresUpdate)
		...
		requiresUpdate = false; //Create JSON
	std::stringstream ss;
	ss << '{';
...
WRAPPER_EFFECT HandleKeyboardEffect(ChromaSDK::Keyboard::EFFECT_TYPE Effect, PRZPARAM pParam)
{
	...
	if (Effect == ChromaSDK::Keyboard::CHROMA_STATIC)
	{
		...
		if (static_effect != NULL)
		{
			...
			{
				if (return_effect.bitmap[colorset] != blue || return_effect.bitmap[colorset + 1] != green || return_effect.bitmap[colorset + 2] != red)
					requiresUpdate = true;
	...
	else if (Effect == ChromaSDK::Keyboard::CHROMA_NONE)
	{
		for (int colorset = 0; colorset < LOGI_LED_BITMAP_SIZE; colorset += 4)
		{
			if (return_effect.bitmap[colorset] != 0 || return_effect.bitmap[colorset + 1] != 0 || return_effect.bitmap[colorset + 2] != 0)
				requiresUpdate = true;
		...
		if (current_logo[0] != 0 || current_logo[1] != 0 || current_logo[2] != 0)
			requiresUpdate = true;
		return_effect.logo[0] = 0;
		return_effect.logo[1] = 0;
		return_effect.logo[2] = 0;
		return_effect.logo[3] = (char)255; //G Keys
			...
			for (int row = 0; row < ChromaSDK::Keyboard::MAX_ROW; row++)
			{
				for (int col = 0; col < ChromaSDK::Keyboard::MAX_COLUMN; col++)
				{
					...
					if (bitmap_pos != Logitech_keyboardBitmapKeys::UNKNOWN)
					{
						...
						{
							if (current_logo[0] != blue || current_logo[1] != green || current_logo[2] != red)
								requiresUpdate = true;
						...
						{
							if (current_g1[0] != blue || current_g1[1] != green || current_g1[2] != red)
								requiresUpdate = true;
						...
						{
							if (current_g2[0] != blue || current_g2[1] != green || current_g2[2] != red)
								requiresUpdate = true;
						...
						{
							if (current_g3[0] != blue || current_g3[1] != green || current_g3[2] != red)
								requiresUpdate = true;
						...
						{
							if (current_g4[0] != blue || current_g4[1] != green || current_g4[2] != red)
								requiresUpdate = true;
						...
						{
							if (current_g5[0] != blue || current_g5[1] != green || current_g5[2] != red)
								requiresUpdate = true;
						...
						{
							if (current_bitmap[(int)bitmap_pos] != blue || current_bitmap[(int)bitmap_pos + 1] != green || current_bitmap[(int)bitmap_pos + 2] != red)
								requiresUpdate = true;
...
WRAPPER_EFFECT HandleMouseEffect(ChromaSDK::Mouse::EFFECT_TYPE Effect, PRZPARAM pParam)
{
	...
	if (Effect == ChromaSDK::Mouse::CHROMA_STATIC)
	{
		...
		if (static_effect != NULL)
		{
			...
			{
				if (current_peripheral[0] != blue || current_peripheral[1] != green || current_peripheral[2] != red)
					requiresUpdate = true;
	...
	{
		if (current_peripheral[0] != 0 || current_peripheral[1] != 0 || current_peripheral[2] != 0)
			requiresUpdate = true;
...
WRAPPER_EFFECT HandleMousepadEffect(ChromaSDK::Mousepad::EFFECT_TYPE Effect, PRZPARAM pParam)
{
	...
	if (Effect == ChromaSDK::Mousepad::CHROMA_STATIC)
	{
		...
		if (static_effect != NULL)
		{
			...
			{
				if (current_mpad[colorset] != blue || current_mpad[colorset + 1] != green || current_mpad[colorset + 2] != red)
					requiresUpdate = true;
	...
	else if (Effect == ChromaSDK::Mousepad::CHROMA_NONE)
	{
		for (int colorset = 0; colorset < 56; colorset += 4)
		{
			if (current_mpad[colorset] != 0 || current_mpad[colorset + 1] != 0 || current_mpad[colorset + 2] != 0)
				requiresUpdate = true;
...
__declspec(dllexport) RZRESULT Init()
{
	...
	if (!isInitialized)
	{
		...
		program_name = filepath.substr(fn_beginning); //Connect to the server pipe using CreateFile()
		hPipe = CreateFile(PIPE_NAME, // pipe name 
		                   GENERIC_WRITE, // write access 
		                   0, // no sharing 
		                   NULL, // default security attributes
...
__declspec(dllexport) RZRESULT QueryDevice(RZDEVICEID DeviceId, ChromaSDK::DEVICE_INFO_TYPE& DeviceInfo)
{
	if (isInitialized)
	{
		...
		else
		{
			...
			return RZRESULT_SUCCESS; //return RZRESULT_DEVICE_NOT_CONNECTED;
		}
