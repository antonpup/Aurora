...
namespace Aurora.Profiles.CSGO
{
    ...
    public class GameEvent_CSGO : GameEvent
    {
        ...
        {
            isDimming = false;
            dim_bg_at = general_timer.ElapsedMilliseconds + (Global.Configuration.csgo_settings.bg_dim_after * 1000L);
            dim_value = 1.0;
        ...
        {
            if (isDimming && Global.Configuration.csgo_settings.bg_enable_dimming)
            {
                dim_value -= 0.02;
        ...
        public void UpdateLights(EffectFrame frame)
        {
            ...
            //update background
            if (Global.Configuration.csgo_settings.bg_team_enabled)
            {
                EffectLayer bg_layer = new EffectLayer("CSGO - Background");

                Color bg_color = Global.Configuration.csgo_settings.ambient_color;

                if (current_team == PlayerTeam.T)
                    bg_color = Global.Configuration.csgo_settings.t_color;
                else if (current_team == PlayerTeam.CT)
                    bg_color = Global.Configuration.csgo_settings.ct_color;
                ...
                if (Global.Configuration.csgo_settings.bg_peripheral_use)
            ...
            if (current_team != PlayerTeam.Undefined)
            {
                ...
                if (Global.Configuration.csgo_settings.health_enabled)
                hpbar_layer.PercentEffect(Global.Configuration.csgo_settings.healthy_color,
                        Global.Configuration.csgo_settings.hurt_color, 
                        Global.Configuration.csgo_settings.health_sequence, 
                        (double)health, 
                        (double)health_max, 
                        Global.Configuration.csgo_settings.health_effect_type);

                layers.Enqueue(hpbar_layer);

                //Update Ammo
                EffectLayer ammobar_layer = new EffectLayer("CSGO - Ammo Bar");
                if (Global.Configuration.csgo_settings.ammo_enabled)
                    ammobar_layer.PercentEffect(Global.Configuration.csgo_settings.ammo_color,
                        Global.Configuration.csgo_settings.noammo_color, 
                        Global.Configuration.csgo_settings.ammo_sequence, 
                        (double)clip, 
                        (double)clip_max, 
                        Global.Configuration.csgo_settings.ammo_effect_type);

                layers.Enqueue(ammobar_layer);


                //Update Bomb
                if (Global.Configuration.csgo_settings.bomb_enabled)
                {
                    EffectLayer bomb_effect_layer = new EffectLayer("CSGO - Bomb Effect");

                    Devices.DeviceKeys[] _bombkeys = Global.Configuration.csgo_settings.bomb_sequence.keys.ToArray();
                    ...
                    if (bombstate == BombState.Planted)
                    {
                        ...
                        if (!Global.Configuration.csgo_settings.bomb_gradual)
                        ...
                        foreach (Devices.DeviceKeys key in _bombkeys)
                        {
                            if (isCritical)
                            {
                                Color bombcolor_critical = Utils.ColorUtils.MultiplyColorByScalar(Global.Configuration.csgo_settings.bomb_primed_color, Math.Min(bombflashamount, 1.0));

                                bomb_effect_layer.Set(key, bombcolor_critical);

                                if (Global.Configuration.csgo_settings.bomb_peripheral_use)
                                {
                            ...
                            {
                                Color bombcolor = Utils.ColorUtils.MultiplyColorByScalar(Global.Configuration.csgo_settings.bomb_flash_color, Math.Min(bombflashamount, 1.0));
                    ...
                    {
                        bombtimer.Stop();
                        if (Global.Configuration.csgo_settings.bomb_display_winner_color)
                        {
                            foreach (Devices.DeviceKeys key in _bombkeys)
                                bomb_effect_layer.Set(key, Global.Configuration.csgo_settings.ct_color);

                            if (Global.Configuration.csgo_settings.bomb_peripheral_use)
                                bomb_effect_layer.Set(Devices.DeviceKeys.Peripheral, Global.Configuration.csgo_settings.ct_color);
                        }
                    }
                    else if (bombstate == BombState.Exploded)
                    {
                        ...
                        {
                            foreach (Devices.DeviceKeys key in _bombkeys)
                                bomb_effect_layer.Set(key, Global.Configuration.csgo_settings.t_color);
                            if (Global.Configuration.csgo_settings.bomb_peripheral_use)
                                bomb_effect_layer.Set(Devices.DeviceKeys.Peripheral, Global.Configuration.csgo_settings.t_color);
                        }
                    }
                    else
                    {
            ...
            //Kills Indicator
            if (Global.Configuration.csgo_settings.kills_indicator && isLocal)
            {
                EffectLayer kills_indicator_layer = new EffectLayer("CSGO - Kills Indicator");
                Devices.DeviceKeys[] _killsKeys = Global.Configuration.csgo_settings.kills_sequence.keys.ToArray();
                    ...
                    if (pos < roundKills.Count)
                    {
                        switch (roundKills[pos])
                        {
                            case (RoundKillType.Regular):
                                kills_indicator_layer.Set(_killsKeys[pos], Global.Configuration.csgo_settings.kills_regular_color);
                                break;
                            case (RoundKillType.Headshot):
                                kills_indicator_layer.Set(_killsKeys[pos], Global.Configuration.csgo_settings.kills_headshot_color);
                                break;
                        }
                    }
                }
            ...
            cz_layer.DrawColorZones(Global.Configuration.csgo_settings.lighting_areas.ToArray());
            layers.Enqueue(cz_layer);

            //Update Burning
            
            if (Global.Configuration.csgo_settings.burning_enabled && burnamount > 0)
            {
                EffectLayer burning_layer = new EffectLayer("CSGO - Burning");
                double burning_percent = (double)burnamount / 255.0;
                Color burncolor = Global.Configuration.csgo_settings.burning_color;

                if (Global.Configuration.csgo_settings.burning_animation)
                {
                    int red_adjusted = (int)(Global.Configuration.csgo_settings.burning_color.R + (Math.Cos((general_timer.ElapsedMilliseconds + randomizer.Next(75)) / 75.0) * 0.15 * 255));
                    byte red = 0;
                    ...
                    int green_adjusted = (int)(Global.Configuration.csgo_settings.burning_color.G + (Math.Sin((general_timer.ElapsedMilliseconds + randomizer.Next(150)) / 75.0) * 0.15 * 255));
                    byte green = 0;
                    ...
                    int blue_adjusted = (int)(Global.Configuration.csgo_settings.burning_color.B + (Math.Cos((general_timer.ElapsedMilliseconds + randomizer.Next(225)) / 75.0) * 0.15 * 255));
                    byte blue = 0;
                ...
                if (Global.Configuration.csgo_settings.burning_peripheral_use)
                    burning_layer.Set(Devices.DeviceKeys.Peripheral, burncolor);
                layers.Enqueue(burning_layer);
            }

            //Update Flashed
            if (Global.Configuration.csgo_settings.flashbang_enabled && flashamount > 0)
            {
                EffectLayer flashed_layer = new EffectLayer("CSGO - Flashed");
                double flash_percent = (double)flashamount / 255.0;

                Color flash_color = Color.FromArgb(flashamount, Global.Configuration.csgo_settings.flash_color);

                flashed_layer.Fill(flash_color);

                if (Global.Configuration.csgo_settings.flashbang_peripheral_use)
                    flashed_layer.Set(Devices.DeviceKeys.Peripheral, flash_color);
                layers.Enqueue(flashed_layer);
            }

            //Update Typing Keys
            if (Global.Configuration.csgo_settings.typing_enabled && current_activity == PlayerActivity.TextInput)
            {
                EffectLayer typing_keys_layer = new EffectLayer("CSGO - Typing Keys");
                Devices.DeviceKeys[] _typingkeys = Global.Configuration.csgo_settings.typing_sequence.keys.ToArray();
                foreach (Devices.DeviceKeys key in _typingkeys)
                    typing_keys_layer.Set(key, Global.Configuration.csgo_settings.typing_color);
        ...
        {
            return Global.Configuration.csgo_settings.isEnabled;
        }
    }
}
