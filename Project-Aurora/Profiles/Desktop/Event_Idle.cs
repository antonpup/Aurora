...
namespace Aurora.Profiles.Desktop
{
    public class Event_Idle : GameEvent
    {
        ...
        public void UpdateLights(EffectFrame frame)
        {
            ...
            effect_cfg.speed = Global.Configuration.desktop_settings.idle_speed;

            switch (Global.Configuration.desktop_settings.idle_type)
            {
                ...
                case IdleEffects.ColorBreathing:
                    ...
                    Color breathe_bg_color = Global.Configuration.desktop_settings.idle_effect_secondary_color;
                    layer.Fill(breathe_bg_color);

                    float sine = (float)Math.Pow(Math.Sin((double)((currenttime % 10000L) / 10000.0f) * 2 * Math.PI * Global.Configuration.desktop_settings.idle_speed), 2);

                    layer.Fill(Color.FromArgb((byte)(sine * 255), Global.Configuration.desktop_settings.idle_effect_primary_color));
                ...
                case IdleEffects.StarFall:
                    ...
                    {
                        for (int x = 0; x < Global.Configuration.desktop_settings.idle_amount; x++)
                        {
                        ...
                        nextstarset = currenttime + (long)(1000L * Global.Configuration.desktop_settings.idle_frequency);
                    }

                    layer.Fill(Global.Configuration.desktop_settings.idle_effect_secondary_color);

                    Devices.DeviceKeys[] stars_keys = stars.Keys.ToArray();

                    foreach (Devices.DeviceKeys star in stars_keys)
                    {
                        layer.Set(star, Utils.ColorUtils.MultiplyColorByScalar(Global.Configuration.desktop_settings.idle_effect_primary_color, stars[star]));
                        stars[star] -= getDeltaTime() * 0.05f * Global.Configuration.desktop_settings.idle_speed;
                    }
                ...
                case IdleEffects.RainFall:
                    ...
                    ColorSpectrum drop_spec = new ColorSpectrum(Global.Configuration.desktop_settings.idle_effect_primary_color, Color.FromArgb(0, Global.Configuration.desktop_settings.idle_effect_primary_color));
                    ...
                    foreach (Devices.DeviceKeys raindrop in raindrops_keys)
                    {
                        ...
                        raindrops[raindrop] -= getDeltaTime() * 0.05f * Global.Configuration.desktop_settings.idle_speed;
                    }
