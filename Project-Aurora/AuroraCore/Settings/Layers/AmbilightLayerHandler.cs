    ...
    public class AmbilightLayerHandler : LayerHandler<AmbilightLayerHandlerProperties>, INotifyPropertyChanged
    {
        ...
        public void Initialize()
        {
            ...
            else
            {
                ...
                catch (SharpDXException e)
                {
                    ...
                    {
                        fallback = true;
                        AuroraCore.logger.Fatal("Desktop Duplication is not supported on this system.\nIf you have multiple graphic cards, try running on integrated graphics.", e);
                    }
                    AuroraCore.logger.Debug(e, String.Format("Caught exception when trying to setup desktop duplication. Retrying in {0} ms", AmbilightLayerHandler.retryTimer.Interval));
                    captureTimer?.Stop();
        ...
        private void CaptureTimer_Elapsed(object sender, System.Timers.ElapsedEventArgs e)
        {
            ...
            else
            {
                ...
                {
                    AuroraCore.logger.Error("Failed to capture screen, reinitializing. Error was: " + err.Message);
                    processing = false;
                    this.Initialize();
                    return;
                }
