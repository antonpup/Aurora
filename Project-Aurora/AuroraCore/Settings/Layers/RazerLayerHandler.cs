...
namespace Aurora.Settings.Layers
{
    ...
    public class RazerLayerHandler : LayerHandler<RazerLayerHandlerProperties>
    {
        ...
        public RazerLayerHandler()
        {
            ...
            if (AuroraCore.Instance.razerManager != null)
            {
                AuroraCore.Instance.razerManager.DataUpdated += OnDataUpdated;

                var appList = AuroraCore.Instance.razerManager.GetDataProvider<RzAppListDataProvider>();
                appList.Update();
        ...
        public bool StartDumpingData()
        {
            ...
            AuroraCore.logger.Info("RazerLayerHandler started dumping data");
            _isDumping = true;
            return true;
        }
        ...
        {
            AuroraCore.logger.Info("RazerLayerHandler stopped dumping data");
            _isDumping = false;
        }

        public void DumpData(AbstractDataProvider provider)
        {
            var path = Path.Combine(Const.LogsDirectory, "RazerLayer");
