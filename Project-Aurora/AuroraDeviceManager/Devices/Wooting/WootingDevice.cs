...
namespace AuroraDeviceManager.Devices.Wooting;
...
public class WootingDevice : DefaultDevice
{
    ...
    protected override Task<bool> UpdateDevice(Dictionary<DeviceKeys, Color> keyColors, DoWorkEventArgs e, bool forced = false)
    {
        ...
        double rScalar = Global.DeviceConfig.VarRegistry.GetVariable<int>($"{DeviceName}_scalar_r") / 100.0;
        double gScalar = Global.DeviceConfig.VarRegistry.GetVariable<int>($"{DeviceName}_scalar_g") / 100.0;
        double bScalar = Global.DeviceConfig.VarRegistry.GetVariable<int>($"{DeviceName}_scalar_b") / 100.0;
        ...
        try
        {
            foreach (var key in keyColors)
            {
                ...
                {
                    var clr = CommonColorUtils.CorrectWithAlpha(key.Value);
