    ...
    public class ScriptedDevice : Device
    {
        ...
        public bool Initialize()
        {
            if (!isInitialized)
            {
                ...
                {
                    if (Global.writeLogFile) Global.logger.LogLine(string.Format("Device script for {0} encountered an error during Initialization. Exception: {1}", devicename, exc), Logging_Level.External);
                    crashed = true;
                    isInitialized = false;
        ...
        public void Reset()
        {
            if (isInitialized)
            {
                ...
                {
                    if (Global.writeLogFile) Global.logger.LogLine(string.Format("Device script for {0} encountered an error during Reset. Exception: {1}", devicename, exc), Logging_Level.External);
                    crashed = true;
                    isInitialized = false;
                }
            }
        ...
        public void Shutdown()
        {
            if (isInitialized)
            {
                ...
                {
                    if (Global.writeLogFile) Global.logger.LogLine(string.Format("Device script for {0} encountered an error during Shutdown. Exception: {1}", devicename, exc), Logging_Level.External);
                    crashed = true;
                    isInitialized = false;
                }
            }
        ...
        public bool UpdateDevice(Dictionary<DeviceKeys, Color> keyColors, bool forced = false)
        {
            if (isInitialized)
            {
                ...
                {
                    if (Global.writeLogFile) Global.logger.LogLine(string.Format("Device script for {0} encountered an error during UpdateDevice. Exception: {1}", devicename, exc), Logging_Level.External);
                    crashed = true;
                    isInitialized = false;
