    ...
    class CorsairDevice : Device
    {
            ...
            lock (action_lock)
            {
                if (!isInitialized)
                {
                    try
                    {
                        ...
                        if (Global.writeLogFile) Global.logger.LogLine("Corsair device, Initialized with " + CueSDK.LoadedArchitecture + "-SDK", Logging_Level.Info);
                    ...
                    {
                        if (Global.writeLogFile) Global.logger.LogLine("Corsair device, CUE Exception! ErrorCode: " + Enum.GetName(typeof(CorsairError), ex.Error), Logging_Level.Error);
                    }
                    catch (WrapperException ex)
                    {
                        if (Global.writeLogFile) Global.logger.LogLine("Corsair device, Wrapper Exception! Message: " + ex.Message, Logging_Level.Error);
                    }
                    catch (Exception ex)
                    {
                        if (Global.writeLogFile) Global.logger.LogLine("Corsair device, Exception! Message: " + ex, Logging_Level.Error);
                    }
        ...
        public void Shutdown()
        {
            lock (action_lock)
            {
                ...
                {
                    if (Global.writeLogFile) Global.logger.LogLine("Corsair device, Exception during Shutdown. Message: " + exc, Logging_Level.Error);
                    isInitialized = false;
                }
            }
        }
        ...
        public bool UpdateDevice(Dictionary<DeviceKeys, Color> keyColors, bool forced = false)
        {
            ...
            {
                if (Global.writeLogFile) Global.logger.LogLine("Corsair device, error when updating device. Error: " + e, Logging_Level.Error);
                Console.WriteLine(e);
                return false;
            }
        }
