...
namespace Aurora.Profiles.QuakeChampions
{
    ...
    /// </summary>
    public partial class Control_QuakeChampions : UserControl
    {
        private Application profile_manager;

        public Control_QuakeChampions(Application profile)
        {
            InitializeComponent();
        ...
        private bool InstallWrapper(string installpath = "")
        {
            ...
            if (!String.IsNullOrWhiteSpace(installpath))
            {
                using (BinaryWriter razer_wrapper_86 = new BinaryWriter(new FileStream(System.IO.Path.Combine(installpath, "client" , "bin" , "pc" , "RzChromaSDK.dll"), FileMode.Create)))
                {
                    razer_wrapper_86.Write(Properties.Resources.Aurora_RazerLEDWrapper86);
                }

                using (BinaryWriter razer_wrapper_64 = new BinaryWriter(new FileStream(System.IO.Path.Combine(installpath, "client" , "bin" , "pc", "RzChromaSDK64.dll"), FileMode.Create)))
                {
        ...
        private bool UninstallWrapper()
        {
            ...
            if (!String.IsNullOrWhiteSpace(installpath))
            {
                string path = System.IO.Path.Combine(installpath, "client", "bin", "pc", "RzChromaSDK.dll");
                string path64 = System.IO.Path.Combine(installpath, "client", "bin", "pc", "RzChromaSDK64.dll");
