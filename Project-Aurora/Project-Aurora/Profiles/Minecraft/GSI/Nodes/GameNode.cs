...
namespace Aurora.Profiles.Minecraft.GSI.Nodes {
    ...
    public class MinecraftKeyBinding {
        ...
        /// <summary>
        /// <para>Method that calculates whether this key conflicts with another.
        /// Returns a MinecraftKeyBindingConflict indicating whether the keys conflict and if so, if a "hard" or "soft" conflict.
        /// Forge shows hard conflicts in red and soft in orange on the in-game keys menu.</para>
        /// <para>- A hard conflict is one in which the keybinds are identical OR have the same key and one has a
        /// modifier and one does not (e.g. "A" and "A" or "B" and "Shift + B").</para>
        /// <para>- A soft conflict is one in which one keybind is a modifier key is assigned as a keybind, and another
        /// keybind uses that modifier key in addition to a regular key (e.g. "Ctrl" and "Ctrl + E").</para>
        /// </summary>
        public MinecraftKeyBindingConflict ConflictsWith(MinecraftKeyBinding other) {
            // Keys can only conflict if they either have the same context OR EITHER has universal context.
            if (context != other.context && context != "UNIVERSAL" && other.context != "UNIVERSAL")
                return MinecraftKeyBindingConflict.None;

            // If the main keys are the same
            if (keyCode == other.keyCode)
                return modifier == other.modifier || modifier == "NONE" || other.modifier == "NONE" // There is a conflict if one doesn't use a modifier or they use the same modifier
                    ? MinecraftKeyBindingConflict.Hard
                    : MinecraftKeyBindingConflict.None;

            // Else if one of the binds uses a modifier key as the main key and the modifier is used as a modifier for other binding
            // 0x2A = LSHIFT, 0x1D = Control, 0x38 = 
            else if ((keyCode == 0x2A && other.modifier == "SHIFT") || (keyCode == 0x1D && other.modifier == "CONTROL") || (keyCode == 0x38 && other.modifier == "ALT")
                  || (other.keyCode == 0x2A && modifier == "SHIFT") || (other.keyCode == 0x1D && modifier == "CONTROL") || (other.keyCode == 0x38 && modifier == "ALT"))
                return MinecraftKeyBindingConflict.Soft;
            
            // If neither of the above, there is no conflict.
            return MinecraftKeyBindingConflict.None;
        }
    ...
    public enum MinecraftKeyBindingConflict { None, Hard, Soft }
}
