...
namespace Aurora
{
    ...
    public static class Global
    {
        ...
        /// <summary>
        /// A boolean indicating whether or not to log to the logfile
        /// </summary>
    ...
    static class Program
    {
            ...
            if (mutex.WaitOne(TimeSpan.Zero, true))
            {
                ...
                for (int arg_i = 0; arg_i < args.Length; arg_i++)
                {
                    ...
                    switch (arg)
                    {
                        case ("-debug"):
                            Global.isDebug = true;
                            if (Global.writeLogFile) Global.logger.LogLine("Program started in debug mode.", Logging_Level.Info);
                            break;
                        case ("-silent"):
                            isSilent = true;
                            if (Global.writeLogFile) Global.logger.LogLine("Program started with '-silent' parameter", Logging_Level.Info);
                            break;
                        case ("-ignore_update"):
                            ignore_update = true;
                            if (Global.writeLogFile) Global.logger.LogLine("Program started with '-ignore_update' parameter", Logging_Level.Info);
                            break;
                        case ("-delay"):
                            isDelayed = true;
                            ...
                            if (Global.writeLogFile) Global.logger.LogLine("Program started with '-delay' parameter with delay of " + delayTime + " ms", Logging_Level.Info);

                            break;
                        case ("-install_logitech"):
                            if (Global.writeLogFile) Global.logger.LogLine("Program started with '-install_logitech' parameter", Logging_Level.Info);
                ...
                if ((processes = Process.GetProcessesByName(Process.GetCurrentProcess().ProcessName)).Length > 1)
                {
                    ...
                    {
                        if (Global.writeLogFile) Global.logger.LogLine("Aurora is already running.", Logging_Level.Error);
                ...
                //Load config
                if (Global.writeLogFile) Global.logger.LogLine("Loading Configuration", Logging_Level.Info);
                try
                {
                ...
                {
                    if (Global.writeLogFile) Global.logger.LogLine("Exception during ConfigManager.Load(). Error: " + e, Logging_Level.Error);
                ...
                if (!Global.Configuration.write_log_file) Global.writeLogFile = false;
                ...
                if (Global.Configuration.updates_check_on_start_up && !ignore_update)
                {
                    ...
                    if (File.Exists(updater_path))
                    {
                        ...
                        {
                            if (Global.writeLogFile) Global.logger.LogLine("Could not start Aurora Updater. Error: " + exc, Logging_Level.Error);
                        }
                    }
                }

                if (Global.writeLogFile) Global.logger.LogLine("Loading Plugins", Logging_Level.Info);
                (Global.PluginManager = new PluginManager()).Initialize();

	            if (Global.writeLogFile) Global.logger.LogLine("Loading KB Layouts", Logging_Level.Info);
				...
				if (Global.writeLogFile) Global.logger.LogLine("Loading Input Hooking", Logging_Level.Info);
                ...
                if (Global.writeLogFile) Global.logger.LogLine("Loading Applications", Logging_Level.Info);
                (Global.LightingStateManager = new LightingStateManager()).Initialize();

                if (Global.writeLogFile) Global.logger.LogLine("Loading Device Manager", Logging_Level.Info);
                ...
                /*if (Global.writeLogFile) Global.logger.LogLine("Starting GameEventHandler", Logging_Level.Info);
                Global.geh = new GameEventHandler();
                if (!Global.geh.Init())
                {
                    if (Global.writeLogFile) Global.logger.LogLine("GameEventHander could not initialize", Logging_Level.Error);
                    return;
                }*/

                if (Global.writeLogFile) Global.logger.LogLine("Starting GameStateListener", Logging_Level.Info);
                try
                {
                ...
                {
                    if (Global.writeLogFile) Global.logger.LogLine("GameStateListener Exception, " + exc, Logging_Level.Error);
                ...
                {
                    if (Global.writeLogFile) Global.logger.LogLine("GameStateListener could not start", Logging_Level.Error);
                    System.Windows.MessageBox.Show("GameStateListener could not start. Try running this program as Administrator.\r\nExiting.");
                    Environment.Exit(0);
                }

                if (Global.writeLogFile) Global.logger.LogLine("Listening for game integration calls...", Logging_Level.None);

                if (Global.writeLogFile) Global.logger.LogLine("Loading WinApp...", Logging_Level.None);
                WinApp = new System.Windows.Application();
                if (Global.writeLogFile) Global.logger.LogLine("Loaded WinApp", Logging_Level.None);

                if (Global.writeLogFile) Global.logger.LogLine("Loading ResourceDictionaries...", Logging_Level.None);
                ...
                if (Global.writeLogFile) Global.logger.LogLine("Loaded ResourceDictionaries", Logging_Level.None);

                WinApp.ShutdownMode = ShutdownMode.OnExplicitShutdown;

                if (Global.writeLogFile) Global.logger.LogLine("Loading ConfigUI...", Logging_Level.None);
        ...
        public static void Exit()
        {
            ...
            {
                if (Global.writeLogFile) Global.logger.LogLine("Exception closing \"Aurora-SkypeIntegration\", Exception: " + exc);
            }
        ...
        private static void CurrentDomain_UnhandledException(object sender, UnhandledExceptionEventArgs e)
        {
            ...
            if (Global.writeLogFile) Global.logger.LogLine("Fatal Exception caught : " + exc, Logging_Level.Error);
            if (Global.writeLogFile) Global.logger.LogLine(String.Format("Runtime terminating: {0}", e.IsTerminating), Logging_Level.Error);
        ...
        public static void InstallLogitech()
        {
            ...
            if (!isElevated)
            {
                if (Global.writeLogFile) Global.logger.LogLine("Program does not have admin rights", Logging_Level.Error);
            ...
            if (Global.writeLogFile) Global.logger.LogLine("Logitech LED SDK patched successfully", Logging_Level.Info);
        ...
        static void OnProcessExit(object sender, EventArgs e)
        {
            ...
            {
                if (Global.writeLogFile) Global.logger.LogLine("Exception during OnProcessExit(). Error: " + exc, Logging_Level.Error);
            }
        }
    }
}
