...
namespace Aurora.Settings.Layers {
    ...
    public class ComparisonLayerHandler : LayerHandler<ComparisonLayerProperties> {
        ...
        public override void SetApplication(Application profile) {
            if (profile != null) {
                if (!double.TryParse(Properties._Operand1Path, out double value) && !string.IsNullOrWhiteSpace(Properties._Operand1Path) && !profile.ParameterLookup.IsValidPath(Properties._Operand1Path))
                    Properties._Operand1Path = string.Empty;
                if (!double.TryParse(Properties._Operand2Path, out value) && !string.IsNullOrWhiteSpace(Properties._Operand2Path) && !profile.ParameterLookup.IsValidPath(Properties._Operand2Path))
